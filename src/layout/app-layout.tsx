...
import { usePharmacyFindFirst } from 'lib/roq';
...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const tenantRoles = ['pharmacist', 'business-owner', 'pharmacy-manager', 'medical-representative'];
  const isTenantUser = tenantRoles.some((role) => session?.user?.roles?.includes(role));
  const { data, error, isLoading } = usePharmacyFindFirst(
  ...
  const MockedLinkItems: Array<NavItemPropsInterface> = [
    ...
    {
      name: 'Pharmacies',
      path: isTenantUser ? `/pharmacies/view/${data?.id}` : '/pharmacies',
      entity: 'pharmacy',
